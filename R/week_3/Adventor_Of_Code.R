...
# magical energy to deliver presents on Christmas. For that, their favorite snack is a special
# type of star fruit that only grows deep in the jungle. The Elves have brought you on their annual
...
read_txt <- readLines("Adventorr.txt")
read_txt
...
for (i in read_txt) {
 if(nchar(i) > 2 & as.numeric(i) > 0){
  # print(i)
   sum_of_num = sum_of_num + as.numeric(i)
 } else{
   all_nums <- append(all_nums, sum_of_num)
   sum_of_num = 0
 }  
}
print(all_nums)

max_value <- max(all_nums, na.rm = FALSE)
max_value
